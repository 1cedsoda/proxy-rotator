...
// Resolve implements proxykit.Handler — returns the least-used proxy.
...
func (s *StaticFileSource) Resolve(ctx context.Context, _ *proxykit.Request) (*proxykit.Result, error) {
...
func loadProxiesFromFile(path string, format ProxyFormat) ([]proxykit.Proxy, error) {
	f, err := os.Open(path)
	if err != nil {
