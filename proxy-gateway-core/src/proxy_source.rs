...
/// across Tokio tasks. The `#[async_trait]` macro makes the async methods
/// dyn-compatible so `Box<dyn ProxySource>` works.
...
pub trait ProxySource: Send + Sync + std::fmt::Debug {
    ...
    /// may fall back to a new proxy if no alternatives exist (e.g. a
    /// single-entry pool).
    ///
    /// The default implementation simply delegates to
    /// [`get_source_proxy`](Self::get_source_proxy).
    async fn get_source_proxy_force_rotate(
        &self,
        affinity_params: &AffinityParams,
        _current: &SourceProxy,
    ) -> Option<SourceProxy> {
        self.get_source_proxy(affinity_params).await
    }
